...
Ejecutar antes el server node main_receptor.js
"""
...
def main():
    ...
    # ENLACE
    trama = calcular_integridad(bits_mensaje, algoritmo, block_size)
    etiqueta = (
        f"Fletcher-{block_size}" if algoritmo == "fletcher" else f"Hamming(m={block_size})"
    )
    print(f"\n[ENLACE] Trama con {etiqueta} ({len(trama)} bits):")
    print(f"  {trama}")
